...
Codex Team Router 是一个 Codex 插件，用来在显式 team 模式下、工程任务开始前先做路由判断。默认只有 prompt 以 `team`、`/team`、`团队模式` 或 `使用 team` 开头时才会进入 router。它会帮助 Codex 判断当前请求应该留在主线程完成，还是应该使用有边界的 executor，或者使用 Codex App 里可见的原生 subagent，例如 planner、executor、reviewer、explorer 和 verifier。
...
- 默认使用 manual team 模式。使用 `team <任务>`、`/team <任务>`、`团队模式 <任务>` 或 `使用 team <任务>` 进入 router。设置 `CODEX_TEAM_ROUTER_MODE=auto` 可以恢复自动识别工程 prompt。
...
- 它不会绕过用户 opt-out、高风险确认或工具可用性 gate。hook marker `CODEX_TEAM_ROUTER_ROUTE_REQUIRED` 对这个插件来说是 team/auto 路由授权，但如果任何 gate 阻止 spawn，仍会 fallback 到主线程。
...
默认 manual 模式下，prompt 需要以 `team`、`/team`、`团队模式` 或 `使用 team` 开头才会进入 router。普通工程 prompt 不会注入 route marker，除非设置 `CODEX_TEAM_ROUTER_MODE=auto`。

router 会按 intent、domain 和 complexity 分类被路由的任务：
...
对于 `standard`、`complex`、`parallel_read` 或 `high_risk` 路由，Codex 应该在实现前先拆解任务。router marker 会授权这个 skill 在用户未 opt-out、高风险确认已满足、原生工具可用时选择可见的原生 subagents。
...
预期：默认 manual 模式下不注入 route-required marker。

```text
team 检查一下这个插件的各个方面，看看还有没有改进和优化的空间。
```

预期：注入 marker，intent 是 `review`，domain 是 `infra`，`authorization=explicit`，并且 `team_route=complex`。
...
```text
/team 创建一个简单的 HTML 计数器页面。
```

预期：注入 marker，`authorization=explicit`，并且 `team_route=standard`。
...
```text
使用 team 查找这个仓库里 hooks 和 doctor 的实现，梳理潜在问题，不要修改文件。
```
...
预期：默认 manual 模式下不注入 route-required marker。设置 `CODEX_TEAM_ROUTER_MODE=auto` 后，注入 marker，并且 `authorization=auto`。

```text
Use planner/executor/reviewer subagents to create a small HTML app.
```

预期：默认 manual 模式下不注入 route-required marker，除非加上 `team` 前缀。设置 `CODEX_TEAM_ROUTER_MODE=auto` 后，注入 marker，并且 `authorization=explicit`。

## Hooks 和环境变量

打包的插件 hooks 是刻意保持轻量的：

- `UserPromptSubmit`：检测显式 team 命令，并注入路由上下文。设置 `CODEX_TEAM_ROUTER_MODE=auto` 后，也会检测可能的工程 prompt。
...
所有机器可读报告都会在顶层包含 `schema_version: 1`。报告契约见 [docs/json-reports.md](docs/json-reports.md)。
